...
var _ = Describe("Flusher", func() {
	...
	Context("When all tips was flushed successfully", func() {
		It("returned value is nil", func() {
			mockRepo.EXPECT().AddTips(gomock.Any(), gomock.Any()).Return([]uint64{}, nil).AnyTimes()
			result = f.Flush(tips)
	...
	Context("When error was occurred while flushing tips", func() {
		It("returned slice of tips that have not been flushed", func() {
			gomock.InOrder(
				mockRepo.EXPECT().AddTips(gomock.Any(), gomock.Any()).Return([]uint64{}, nil).Times(batchesCount/3),
				mockRepo.EXPECT().AddTips(gomock.Any(), gomock.Any()).Return(nil, errors.New("unknown error")).Times(batchesCount/3),
				mockRepo.EXPECT().AddTips(gomock.Any(), gomock.Any()).Return([]uint64{}, nil).AnyTimes(),
			)
			result = f.Flush(tips)
